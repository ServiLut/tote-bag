...
export default function UsersManagementPage() {
  ...
  const handleRoleChange = async (userId: string, newRole: string) => {
    ...
    if (
      ...
      toast.error('No puedes quitarte a ti mismo el rol ADMIN');
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorBody?.message || 'Error al actualizar rol');
      }
    ...
    <div className="p-8 md:p-12 space-y-10 max-w-6xl mx-auto">
                    ...
                    <div className="flex justify-end items-center gap-2">
                      {updatingId === user.id ? (
                        ...
                        <Select
                          ...
                          disabled={currentUserId === user.id && user.role === 'ADMIN'}
                        >
      ...
      <div className="flex items-center gap-4 p-6 bg-primary/5 rounded-3xl border border-primary/10">
        ...
        <p className="text-xs font-bold text-primary/70 leading-relaxed">
          <span className="font-black">Zona de Seguridad:</span> Los cambios en los roles afectan
          inmediatamente los permisos del usuario en la plataforma. No puedes quitarte a ti mismo
          el rol ADMIN ni degradar al ultimo ADMIN activo.
        </p>
      </div>
    </div>
  );
