...
const DEFAULT_PRODUCT_ATTRIBUTES: AttributeData[] = [
  ...
  { type: 'LINE', value: '', priceModifier: 0, sortOrder: 3 },
];
...
export const AdminProductForm = ({ initialData }: AdminProductFormProps) => {
    ...
    initialData
      ? {
          ...
          attributes: initialData.attributes && initialData.attributes.length > 0
            ? initialData.attributes.filter((attribute) => attribute.type !== 'QUALITY')
            : [],
  ...
  const discount = formData.comparePrice > formData.basePrice && formData.comparePrice > 0
    ...
    const handleCurrencyFieldChange = (
      field: 'basePrice' | 'comparePrice' | 'costPrice' | 'minPrice',
    ) => (event: ChangeEvent<HTMLInputElement>) => {
      const sanitizedValue = sanitizeDecimalInput(event.target.value);

      setFormData((prev) => ({
        ...prev,
        [field]: sanitizedValue ? parseLocalizedNumber(sanitizedValue) : 0,
      }));
    };
    ...
    const handlePricingRuleIntegerChange =
      (index: number, field: 'minQty' | 'discountPct') => (event: ChangeEvent<HTMLInputElement>) => {
        ...
        if (sanitizedValue === '') {
          updatePricingRule(index, field, field === 'discountPct' ? undefined : 0);
          return;
        }
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  name="basePrice"
                  inputMode="decimal"
                  value={formData.basePrice === 0 ? '' : formatCurrencyInput(String(formData.basePrice))}
                  onChange={handleCurrencyFieldChange('basePrice')}
                  placeholder="0.00"
                  className={cn(
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  name="comparePrice"
                  inputMode="decimal"
                  value={formData.comparePrice === 0 ? '' : formatCurrencyInput(String(formData.comparePrice))}
                  onChange={handleCurrencyFieldChange('comparePrice')}
                  placeholder="0.00"
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  name="costPrice"
                  inputMode="decimal"
                  value={formData.costPrice === 0 ? '' : formatCurrencyInput(String(formData.costPrice))}
                  onChange={handleCurrencyFieldChange('costPrice')}
                  placeholder="0.00"
                  className="w-full block pl-7 p-3 border border-theme rounded-xl bg-base text-primary font-bold focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                />
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  name="minPrice"
                  inputMode="decimal"
                  value={formData.minPrice === 0 ? '' : formatCurrencyInput(String(formData.minPrice))}
                  onChange={handleCurrencyFieldChange('minPrice')}
                  placeholder="0.00"
                  className="w-full block pl-7 p-3 border border-theme rounded-xl bg-base text-primary font-bold focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                  required
                />
        ...
        <section className="space-y-4">
          ...
          <div className="space-y-3">
            {formData.attributes.map((attr, index) => {
              ...
              const isLocked = !isEditMode && index < 3;
