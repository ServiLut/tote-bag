...
export default function WizardConfigManager() {
  ...
  const handleBasePriceModifierChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const sanitizedValue = sanitizeDecimalInput(event.target.value);
    setFormData((prev) => ({
      ...prev,
      basePriceModifier: sanitizedValue ? parseLocalizedNumber(sanitizedValue) : 0,
    }));
  };
  ...
  const handleSortOrderChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData((prev) => ({
      ...prev,
      sortOrder: sanitizedValue === '' ? 0 : Number.parseInt(sanitizedValue, 10),
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const path = editingId ? `/wizard-options/${editingId}` : '/wizard-options';
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      const res = await apiFetch(`/wizard-options/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="space-y-6">
      ...
      {showModal && (
        ...
        >
          <div
            className="bg-surface w-full max-w-lg rounded-3xl border border-theme shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        type="text"
                        inputMode="decimal"
                        value={formData.basePriceModifier === 0 ? '' : formatCurrencyInput(String(formData.basePriceModifier))}
      ...
      {/* Delete Confirmation */}
      {showConfirmDeleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={() => setShowConfirmDeleteId(null)}>
          <div className="bg-surface w-full max-w-md rounded-3xl border border-theme p-8 text-center space-y-6 animate-in zoom-in-95" onClick={(event) => event.stopPropagation()}>
