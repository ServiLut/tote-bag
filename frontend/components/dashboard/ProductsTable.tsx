...
export default function ProductsTable() {
  ...
  const isLowMargin = (base: number, min: number) => {
    return base < min * 1.10;
  };
        ...
        <table className="w-full divide-y divide-zinc-200 text-sm">
          ...
          <tbody className="divide-y divide-zinc-100">
            {products.map((product) => {
              const risk = isLowMargin(product.basePrice, product.minPrice);
              const mainImage = product.images?.[0] || 'https://via.placeholder.com/100';
              ...
              return (
                <tr 
                  ...
                  className={cn(
                    ...
                    risk && "bg-red-50/30 hover:bg-red-50/50"
                  )}
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-zinc-500 font-medium uppercase tracking-wider">{product.collection}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-right">
                    {risk ? (
                      <div className="inline-flex items-center text-red-600 gap-1 bg-red-100/50 px-2 py-1 rounded-md" title="Margen reducido (<10% sobre PMA)">
                        <AlertTriangle className="w-3 h-3" />
                        <span className="font-bold text-[10px] uppercase">Riesgo</span>
                      </div>
                    ) : (
                      <div className="inline-flex items-center text-green-600 gap-1 bg-green-100/50 px-2 py-1 rounded-md">
                        <Check className="w-3 h-3" />
                        <span className="font-bold text-[10px] uppercase">OK</span>
                      </div>
                    )}
                  </td>
                  <td className="px-6 py-4 text-right">
