...
export default function NewManualOrderPage() {
  ...
  const fetchMunicipalities = useCallback(async (departmentId: string) => {
    ...
    try {
      const res = await fetch(`${API_URL}/locations/municipalities/${departmentId}`);
      if (res.ok) {
    ...
    return (
      <div className="mx-auto max-w-2xl space-y-8 p-8 text-center md:p-12">
        ...
        <div className="grid gap-3">
          <button onClick={() => window.open(`${API_URL}/orders/${createdOrder.id}/receipt`, '_blank')} className="rounded-2xl bg-primary p-4 font-black uppercase tracking-widest text-base-color">
          ...
          }} className="rounded-2xl bg-emerald-500 p-4 font-black uppercase tracking-widest text-white">
            <span className="flex items-center justify-center gap-2"><Share2 className="h-5 w-5" />Enviar por WhatsApp</span>
          </button>
        </div>
      </div>
    );
              ...
              {items.length === 0 ? (
                ...
                <div key={item.variantId} className="flex items-center justify-between gap-4 rounded-2xl border border-theme p-4">
                  ...
                  <div className="flex items-center gap-3">
                    <input type="number" min="1" max={item.stock} value={item.quantity} onChange={(event) => updateItemQty(index, parseInt(event.target.value, 10) || 1)} className="w-16 rounded-lg border border-theme bg-base px-2 py-1 text-center font-bold" />
        ...
        <aside className="space-y-4 rounded-3xl border border-theme bg-surface p-6 shadow-lg">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="space-y-2 rounded-2xl border border-theme bg-base/50 p-4">
              ...
              <input type="number" min="0" value={discount} onChange={(event) => setDiscount(Math.max(0, parseFloat(event.target.value) || 0))} className="w-full rounded-xl border border-theme bg-surface px-3 py-2 text-right font-black outline-none focus:ring-2 focus:ring-primary/20" />
              {discount > 0 && <div className="flex items-center justify-between text-rose-500"><span className="text-[10px] font-black uppercase tracking-widest">Descuento Aplicado</span><span className="font-black">-{formatCurrency(discountAmount)}</span></div>}
            </div>
